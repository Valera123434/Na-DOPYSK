...
import org.example.lexer.Types;
...
public class Parser {
    ...
    private void templateFunctions()
    {
        ...
        {
            if ( tokens.get(i).getType() == Types.FUNC )
            {
                Function function;
                function = new Function ( tokens.get(i+1).getValue(), -1);
                i++; // shoot over the (
                while ( tokens.get(i).getType() == Types.VARIABLE)
                {
            ...
            throws Exception
    {
        ...
        while(pos < tokens.size())
        {
            if (match(Types.WRITE))
            {
                statements.add( new Print ( op() ) );
            }
            else if ( match(Types.READ))
            {
                // consume the opening para
                consume(Types.LPARA);
                // create the read with the variable to store it in
                statements.add ( new Input ( consume(Types.VARIABLE).getValue()));
                // consume the trailing para
                consume(Types.RPARA);
            }
            else if ( match(Types.VARIABLE, Types.ASSIGN))
            {
            ...
            else if ( match(Types.VARIABLE, Types.LPARA))
            {
                // match the function call
                matchFunctionCall();
            }
            else if ( match(Types.LABEL))
            {
            ...
            else if ( match(Types.FUNC) )
            {
                ...
                {
                    Function function;
                    function = new Function ( consume(Types.FUNC).getValue(),
                            statements.size() );
            ...
            throws Exception
    {
        ...
        if ( lookAhead(0).getType() == Types.LPARA)
            consume(Types.LPARA);
        ...
        while(match(Types.VARIABLE) || match(Types.INT))
        {
    ...
    // one idx
    private boolean match (Types tag )
    {
        if ( tokens.get(pos).getType() != tag )
            return false;
    ...
    // some time.
    //
    private boolean match ( Types t1, Types t2)
    {
        if ( lookAhead(0).getType() != t1 )
            return false;
        if ( lookAhead(1).getType() != t2 )
            return false;
    ...
    // far away!
    private Token consume (Types tag )
            throws Exception
    {
        if ( tokens.get(pos).getType() != tag )
            throw new Exception();
    ...
    private boolean matchOp()
    {
        ...
        if ( pos < tokens.size() )
        {
            ...
            if ( match(Types.PLUS) || match(Types.MINUS) ||
                    match(Types.MULTIPLY) || match(Types.DIVIDE) ||
                    match(Types.MOD) || match(Types.ASSIGN))
            {
                return true;
            }
        }
            ...
            throws Exception
    {
        ...
        else if ( match(Types.INT))
        {
            return new IntType(Integer.parseInt(lookBack(1).getValue()));
        }
        else if ( match(Types.STRING))
        {
            return new StringType(lookBack(1).getValue());
        }
        else if ( match(Types.EQ) || match(Types.LT) || match(Types.NE) ||
                match(Types.GT) || match(Types.GE) || match(Types.LE))
        {
            ...
            Expression right = getVar();
            consume(Types.RPARA);
            return new FuncEval(left, right, lookBack(5).getType() );
        }
        else if (match(Types.LPARA))
        {
    ...
    public class FuncEval implements Expression
    {
        ...
        private Expression right;
        private Types tag;
        public FuncEval (Expression left, Expression right, Types tag)
        {
            this.left = left;
            this.right = right;
            this.tag = tag;
    ...
    public class FunctionCall implements Statement
    {
        ...
        public void execute()
        {
            ...
            while ( !call_vars.empty() && call_vars.peek() != null )
            {
                ...
                else if ( call_var.getType() == Types.VARIABLE )
                {
